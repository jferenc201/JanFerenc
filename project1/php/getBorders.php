...
$iso2to3 = [
    ...
    'CO'=>'COL','KM'=>'COM','CG'=>'COG','CD'=>'COD','CR'=>'CRI','HR'=>'HRV',
    ...
    'LR'=>'LBR','LY'=>'LBY','LI'=>'LIE','LT'=>'LTU','LU'=>'LUX','MG'=>'MDG',
    ...
    'NG'=>'NGA','NO'=>'NOR','OM'=>'OMN','PK'=>'PAK','PW'=>'PLW','PA'=>'PAN',
...
if (!$json) {
    echo json_encode(['error' => 'GeoJSON not found', 'tried' => $paths]);
    exit;
}

$data = json_decode($json, true);
foreach ($data['features'] as $feature) {
    if (isset($feature['id']) && strtoupper($feature['id']) === $lookup) {
        echo json_encode($feature); exit;
    }
    if (isset($feature['properties']['iso_a2']) && strtoupper($feature['properties']['iso_a2']) === $code) {
        echo json_encode($feature); exit;
    }
}
